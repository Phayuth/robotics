...
model.set_expression(expr_name='cost', expr=sum1(_x**2)) # stage and terminal cost of our control problem. later used as the cost in the MPC formulation
model.setup()
...
mpc.bounds['lower', '_x', 'x'] = -max_x # lower bounds of the states
mpc.bounds['upper', '_x', 'x'] = max_x  # upper bounds of the states
mpc.bounds['lower', '_u', 'u'] = -0.5   # lower bounds of the input
mpc.bounds['upper', '_u', 'u'] = 0.5    # upper bounds of the input
mpc.setup()

# Estimator
estimator = do_mpc.estimator.StateFeedback(model) # We assume, that all states can be directly measured (state-feedback)
...
simulator.x0 = x0
estimator.x0 = x0
mpc.set_initial_guess() # Use initial state to set the initial guess.
