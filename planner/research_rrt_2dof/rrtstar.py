...
class rrt_star():
    def __init__(self, map:np.ndarray, x_start:node, x_goal:node, obs:list, obstacle_center:list, collision_range:float, eta:float=None, maxiteration:int=1000):
        """General RRT star method of planning with Uniform sampling

        Args:
            map (np.ndarray): occupancy grid map
            x_start (node): start configuration node
    ...
    def Sampling(self)-> node:
        ...
        Returns:
            node: random uniform sampling node
        """
    ...
    def Get_Path(self)-> list:
        ...
        n = 0
        for i in self.nodes:
            if self.Distance_Cost(i, self.x_goal) <= self.eta: # 3
    ...
    def start_planning(self):
        ...
        while True:
            ...
            while True:
                ...
                if not self.Node_collision_check(x_new):
                    break
...
if __name__=="__main__":
    ...
    x_start = np.array([3,27]).reshape(2,1)
    x_goal = np.array([27,3]).reshape(2,1)

    
    # SECTION - planning
    rrt = rrt_star(map, x_start, x_goal, obs, obstacle_center, collision_range, maxiteration=1000)
    rrt.start_planning()
    path = rrt.Get_Path()
