...
class rrt_star:
    ...
    def ingoal_region(self, x_new):
        if np.linalg.norm([self.x_goal.x - x_new.x, self.x_goal.y - x_new.y]) <= 20 :# self.eta:
            return True
        else:
            return False
    ...
    def add_parent(self, x_new: node, x_nearest: node) -> node:
        ...
        for x_near in self.nodes:
            ...
            x_new.parent = x_min
            x_new.cost = c_min
            x_new.costr = x_near.costr + self.distance_cost(x_near, x_new)
    ...
    def start_planning(self):
        while True:
            ...
            #     self.graph_data = np.append(
            #         self.graph_data,
            #         np.array([[self.graph_sample_num, graph_cost]]),
            #         axis=0,
            #     )
