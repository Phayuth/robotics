...
class RRTPlotter:
    ...
    def plot_2d_obstacle(robotEnvClass, axis):
        joint1Range = np.linspace(robotEnvClass.configLimit[0][0], robotEnvClass.configLimit[0][1], 360)
        joint2Range = np.linspace(robotEnvClass.configLimit[1][0], robotEnvClass.configLimit[1][1], 360)
        collisionPoint = []
        for theta1 in joint1Range:
            for theta2 in joint2Range:
                config = np.array([[theta1], [theta2]])
                result = robotEnvClass.collision_check(config)
                if result is True:
    ...
    def plot_2d_config_single_tree(plannerClass, path, ax):
        RRTPlotter.plot_2d_obstacle(plannerClass.robotEnvClass, ax)
