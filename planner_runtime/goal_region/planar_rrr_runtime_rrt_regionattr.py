""" Path Planning for Planar RRR with RRT at runtime and Region EndEffector Attraction
"""
...
class RuntimeRRTBase():
    ...
    def planning(self):
        for itera in range(self.maxIteration):
            ...
            if self.is_config_in_collision(xNew) or self.is_connect_config_possible(xNearest, xNew):
                ...
                if self.ee_in_goal(xNew):
                    self.eeInGoalRegion.append(xNew)
                    break

        path = self.search_path()
        xApp = self.nearest_node(self.xGoal)
        seg = 5
        for i in range(1, seg):
            rateTheta1 = (self.xGoal.x - xApp.x)/seg
            rateTheta2 = (self.xGoal.y - xApp.y)/seg
            rateTheta3 = (self.xGoal.z - xApp.z)/seg
            theta1Int = xApp.x + (rateTheta1*i)
            theta2Int = xApp.y + (rateTheta2*i)
            theta3Int = xApp.z + (rateTheta3*i)
            xIntPath = Node(theta1Int, theta2Int, theta3Int)
            path.append(xIntPath)
    ...
    def ee_in_goal(self, xNew):
        thetaGoal = np.array([self.xGoal.x, self.xGoal.y, self.xGoal.z]).reshape(3, 1)
        taskGoalPose = self.robot.forward_kinematic(thetaGoal)
        theta = np.array([xNew.x, xNew.y, xNew.z]).reshape(3, 1)
        taskPose = self.robot.forward_kinematic(theta)

        if np.linalg.norm([taskGoalPose[0,0] - taskPose[0,0], taskGoalPose[1,0] - taskPose[1,0]]) <= 0.9:
            return True
...
if __name__ == "__main__":
    ...
    xGoal = np.array([1.9, -1.2, -1.1]).reshape(3, 1)
    ...
    # planner = RuntimeRRTBase(robot, obsList, thetaInit, thetaApp, eta=0.3, maxIteration=1000)
    # planner.planning()
    # path = planner.search_path()
