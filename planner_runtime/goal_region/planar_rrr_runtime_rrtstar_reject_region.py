""" Path Planning for Planar RRR with RRT star at runtime with rejection region near goal
"""
...
class RuntimeRRTStar():
    ...
    def __init__(self, robot, taskMapObs, xStart, xGoal, xApp, eta=0.3, maxIteration=1000) -> None:
        ...
        self.xApp = Node(xApp[0, 0], xApp[1, 0], xApp[2, 0])
    ...
    def search_path(self):
        XNear = self.near(self.xApp, 1)
        for xNear in XNear:
            if self.is_connect_config_possible(xNear, self.xApp):
                continue
...
if __name__ == "__main__":
    ...
    planner = RuntimeRRTStar(robot, obsList, thetaInit, thetaGoal, thetaApp, eta=0.3, maxIteration=5000)
    planner.planning()
