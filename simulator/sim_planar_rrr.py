...
class RobotArm2DRRRSimulator:
    ...
    def get_cspace_grid(self):
        raise NotImplementedError(
            "This robot is 3DOF. It is difficult to view the 3D grid."
        )
    ...
    def play_back_path(self, path, animation):
        ...
        def update(frame):
            link = self.robot.forward_kinematic(
                path[:, frame].reshape(3, 1), return_link_pos=True
            )
            robotLinks.set_data(
        ...
        animation = animation.FuncAnimation(
            fig, update, frames=path.shape[1], interval=100, repeat=False
        )
        plt.show()
