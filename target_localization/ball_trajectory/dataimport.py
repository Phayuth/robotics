...
# Original data
f4 = open('target_localization/ball_trajectory/newposition.txt', 'r')
p_position = f4.readlines()
p_position = [x.strip("\n[]") for x in p_position]
p_position = [y.split(' ') for y in p_position]
parray = np.array(p_position).astype(np.float64)
print(f"==>> parray.shape: \n{parray.shape}")
...
# plt.plot(r, parabolic(r, *popt), 'r-', label='Fitted Curve')
