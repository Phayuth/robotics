...
class Node:
    ...
    def __repr__(self) -> str:
        return f'\nconfig = [{self.x:.7f}, {self.y:.7f}, {self.z:.7f}, hasParent = {True if self.parent != None else False}, NumChild = {len(self.child)}]'

class CopSim3DRRTComponent:
    ...
    def steer(self, xNearest, xRand, toDistance):
        distX, distY, distZ = self.distance_each_component_between_config(xNearest, xRand)
        dist = np.linalg.norm([distX, distY, distZ])
        if dist <= toDistance:
            xNew = Node(xRand.x, xRand.y, xRand.z)
        else:
    ...
    def is_connect_config_in_region_of_config(self, xToCheckStart, xToCheckEnd, xCenter, radius, NumSeg=10):
        distX, distY, distZ = self.distance_each_component_between_config(xToCheckStart, xToCheckEnd)
        rateX = distX / NumSeg
        rateY = distY / NumSeg
        rateZ = distZ / NumSeg
    ...
    def is_config_in_collision(self, xNew):
        ...
        xNewExtend = np.array([xNew.x, xNew.y, xNew.z, self.joint4Fixed, self.joint5Fixed, self.joint6Fixed]).reshape(6, 1)
        result = self.copHandle.collsion_check(xNewExtend)
    ...
    def is_connect_config_in_collision_fixed_nseg(self, xNearest, xNew, NumSeg=10):
        ...
        distX, distY, distZ = self.distance_each_component_between_config(xNearest, xNew)
        rateX = distX / NumSeg
        rateY = distY / NumSeg
        rateZ = distZ / NumSeg
        for i in range(1, NumSeg - 1):
    ...
    def plot_single_tree(self, tree, path, axis, obstacle_plot=False):
        for vertex in tree:
            if vertex.parent == None:
                ...
                axis.plot([vertex.x, vertex.parent.x], [vertex.y, vertex.parent.y], [vertex.z, vertex.parent.z], color="darkgray")
        ...
        if obstacle_plot is True:  # for obstacle space
            ...
            for theta1 in jointRange:
                for theta2 in jointRange:
                    for theta3 in jointRange:
                        print(theta1, theta2, theta3)
                        xNewExtend = np.array([theta1, theta2, theta3, self.joint4Fixed, self.joint5Fixed, self.joint6Fixed]).reshape(6, 1)
                        result = self.copHandle.collsion_check(xNewExtend)
                        if result is True:
                            xyz_points.append([theta1, theta2, theta3])

            xyz_points = np.array(xyz_points)
            axis.plot(xyz_points[:, 0], xyz_points[:, 1], xyz_points[:, 2], color='darkcyan', linewidth=0, marker='o', markerfacecolor='darkcyan', markersize=1.5)

        if path:
            axis.plot([node.x for node in path], [node.y for node in path], [node.z for node in path], color='blue', linewidth=2, marker='o', markerfacecolor='plum', markersize=5)

        axis.plot(self.xStart.x, self.xStart.y, self.xStart.z, color="blue", linewidth=0, marker='o', markerfacecolor='yellow')
        axis.plot(self.xApp.x, self.xApp.y, self.xApp.z, color="blue", linewidth=0, marker='o', markerfacecolor='green')
        axis.plot(self.xGoal.x, self.xGoal.y, self.xGoal.z, color="blue", linewidth=0, marker='o', markerfacecolor='red')
