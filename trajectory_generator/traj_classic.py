...
class CubicViaPoint:
    ...
    def cubic_3_via_point(theta_0, theta_v, theta_g, tf):
        # case tf = tf1 = tf2
        a10 = theta_0
        a11 = 0
...
if __name__ == "__main__":
    ...
    cubicCoeff = CubicPolynomial.cubic_coef_specify_velo(posStart=0, veloStart=0.5, posFinal=5, veloFinal=0, tF=tF)
    q, dq, ddq = CubicPolynomial.cubic_polynomial(*cubicCoeff, t)

    # cubic multi seg
    t0 = np.arange(0, 2, 0.01)
    t1 = np.arange(2, 4, 0.01)
    t = np.hstack((t0, t1))
    theta0, thetaDot0, thetaDDot0 = CubicPolynomial.cubic_polynomial(a0=5, a1=0, a2=15 / 2, a3=-5 / 2, t=t0)
    theta1, thetaDot1, thetaDDot1 = CubicPolynomial.cubic_polynomial(a0=15, a1=0, a2=-75 / 4, a3=25 / 4, t=t0)
    q = np.hstack((theta0, theta1))
    dq = np.hstack((thetaDot0, thetaDot1))
    ddq = np.hstack((thetaDDot0, thetaDDot1))

    t = np.arange(0, 10, 0.01)
    seg1_theta, seg1_theta_dot, seg1_theta_ddot, seg2_theta, seg2_theta_dot, seg2_theta_ddot = CubicViaPoint.cubic_3_via_point(theta_0=0, theta_v=5, theta_g=10, t_f=10)
    tt = np.arange(0, 2 * 10, 0.01)
