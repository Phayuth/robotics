...
def get_derivative(data, start, end, diff_rule=1):
    ...
    data_dot[..., 0, :] += cfg.diff_rule[diff_rule - 1][idx_middle - 1] * start / (cfg.time_interval**diff_rule)
    data_dot[..., -2, :] += cfg.diff_rule[diff_rule - 1][idx_middle + 1] * end / (cfg.time_interval**diff_rule)
    data_dot[..., -1, :] += cfg.diff_rule[diff_rule - 1][idx_middle] * end / (cfg.time_interval**diff_rule)
    return data_dot[..., :-1, :]  #


def get_diff_matrix(n, diff_rules, time_interval, diff_rule_length=7, order=1, with_end=True):
...
def get_derivative_torch(data, start, end, diff_rule=1):
    ...
    idx_middle = int(cfg.diff_rule_length / 2)
    data_dot[..., 0, :] += cfg.diff_rule_torch[diff_rule - 1][idx_middle - 1] * start / (cfg.time_interval**diff_rule)
    data_dot[..., -2, :] += cfg.diff_rule_torch[diff_rule - 1][idx_middle + 1] * end / (cfg.time_interval**diff_rule)
    data_dot[..., -1, :] += cfg.diff_rule_torch[diff_rule - 1][idx_middle] * end / (cfg.time_interval**diff_rule)
    return data_dot[..., :-1, :]  #
...
def make_video_writer(name, ratio=-1):
    ...
    return cv2.VideoWriter(name, fourcc, 10.0, (int(ratio) * cfg.window_width, cfg.window_height))
...
def get_global_param(steps=cfg.timesteps):
    ...
    cfg.diff_rule = np.array([[0, 0, -1, 1, 0, 0, 0],
                              [0, 0, 1, -2, 1, 0, 0],
                              [0, -0.5, 1, 0, -1, 0.5, 0]])
    cfg.diff_matrices = [
        get_diff_matrix(
    ...
    cfg.diff_matrices_torch = torch.from_numpy(np.array(cfg.diff_matrices)).cuda().float()
