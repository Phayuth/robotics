...
def is_config_in_joint_limit(xCheck):
    if np.all((np.full((2,1),-np.pi) < xCheck) & (xCheck < np.full((2,1),np.pi))):
        return True
    else:
        return False
...
qabw = find_nearest_qb_to_qa(qa, qb)
qbaw = find_nearest_qb_to_qa(qb, qa)

plt.plot([qa[0]], [qa[1]], "*b", label="qa")
plt.plot([qb[0]], [qb[1]], "*r", label="qb")


plt.plot([qa[0], qb[0]], [qa[1], qb[1]], "-", label="normal")

plt.plot([qa[0], qabw[0]], [qa[1], qabw[1]], "g", label="torus")
plt.plot([qb[0], qbaw[0]], [qb[1], qbaw[1]], "g")
