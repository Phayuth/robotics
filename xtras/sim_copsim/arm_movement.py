...
jointHandles = [sim.getObject('/' + 'UR5e_dynamic' + ''.join(jointNames[0:i+1])) for i in range(len(jointNames))]
...
# t=0.09
# conf=sim.getPathInterpolatedConfig(path,times,t)
# print(f"> conf: {conf}")

p = PreRecordedPath.path.T # shape(6, 12)
path = p.T.flatten().tolist()
tfFromStart = 5
times = np.linspace(0, tfFromStart, p.shape[1]).tolist()


def applyJointTargetPositions(handles, vals):
    for i in range(6):
...
try:
    ...
    while t<times[-1]:
        conf=sim.getPathInterpolatedConfig(path,times,t)
        applyJointTargetPositions(jointHandles, conf)
...
#             id=table.remove(movementToExecute,1)
#             movementData=allMovementData[id]
